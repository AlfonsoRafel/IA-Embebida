...
def save_event(event_list, data, fs, t):
    savedir = r'C:\Users\alfon\Desktop\Projects (On Going)\02. Posgrado CEIA\08. Bimestre VIII\Inteligencia Artificial Embebida\predict\event_detection'
...
def predict(filedir):
    fs, data_raw = wavfile.read(join('r',filedir), False)
    Ts = 1/fs
    N = data_raw.shape[0]  
    t = np.arange(0,Ts*N,Ts)
    data = data_raw.astype(np.float32)
    ...
    # load json
    json_file = r'C:\Users\alfon\Desktop\Projects (On Going)\02. Posgrado CEIA\08. Bimestre VIII\Inteligencia Artificial Embebida\experiments\detection.json'
...
def train(datadir):
    exp_path = r'C:\Users\alfon\Desktop\Projects (On Going)\02. Posgrado CEIA\08. Bimestre VIII\Inteligencia Artificial Embebida\experiments'
    expdir = exp_path
